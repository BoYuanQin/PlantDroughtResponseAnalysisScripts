#conda activate Three_Algorithms1
...
def parseArgs(args):
    ...
    解析命令行参数。
    参数:
    args: list, 通常是 sys.argv，包含了从命令行传入的参数。
    
    返回:
    tuple: 包含解析后的选项(opts)和剩余参数(args)。
    """
    parser = OptionParser()
...
def main(args):
    """
    主函数，用于读取表达数据，设置计算环境，并运行选定的算法。
    参数:
    args: list, 命令行参数列表。
    """
    opts, args = parseArgs(args)
    inDF = pd.read_csv(opts.inFile, sep = '\t', index_col = 0, header = 0).T
    # gene_names = inDF.index.tolist()
    inDF = inDF.dropna()
    local_cluster = LocalCluster()
    client = Client(local_cluster)    
    
    if opts.algo == 'GENIE3':
        network = genie3(inDF.to_numpy(), client_or_address = client, gene_names = inDF.columns,seed=123)
        network.to_csv(opts.outFile, index = False, sep = '\t')

    elif opts.algo == 'GRNBoost2':
        network = grnboost2(inDF.to_numpy(), client_or_address = client, gene_names = inDF.columns,seed=123)
        network.to_csv(opts.outFile, index = False, sep = '\t')
