...
class DataProcessor:
    ...
    """
    初始化DataProcessor类的实例。
    参数:
    file_list: list, 包含数据文件路径的列表。
    """
        self.file_list = file_list
    
    def process_data(self, file_name):
    """
    处理单个数据文件，计算Z-score和P-value，并保存显著性结果。
    参数:
    file_name: str, 数据文件的路径。
    """
    ...
    """
    寻找所有处理文件中显著数据的交集，并计算交集数据的均值，保存最终结果。
    """
        ...
        output_file = f"OutputData/ZM/GENIE3NetworkAll/intersection_network.txt"
        intersection_mean.to_csv(output_file, sep='\t', index=False)

if __name__ == "__main__":
    file_list = [ 'OutputData/ZM/GENIE3NetworkAll/Network_GENIE3.tsv']
